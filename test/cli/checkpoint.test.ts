...
describe('handleCheckpointCommand', () => {
  ...
  it('should create checkpoint with description and files', async () => {
    ...
    expect(mockManager.createCheckpoint).toHaveBeenCalledWith('Test checkpoint', ['file1.txt', 'file2.txt'])
  })
  ...
  it('should show no changes message when diff is empty', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith('\\x1b[32m🎉 No changes detected since checkpoint!\\x1b[0m')
  })
  ...
  it('should create auto checkpoint with common files when none specified', async () => {
    ...
    expect(mockManager.createAutoCheckpoint).toHaveBeenCalledWith(['package.json', 'lib', 'index.js', 'index.ts', 'main.js', 'main.ts'])
    expect(log).toHaveBeenCalledWith('✅ Auto-checkpoint created: auto-id')
  })
