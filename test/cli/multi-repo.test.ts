...
describe('multi-repo CLI', () => {
  ...
  describe('help command', () => {
    it('should display help when no command provided', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Multi-Repository Context Management'))
  ...
  describe('add command', () => {
    ...
    it('should show auto-detected relationships', async () => {
      ...
      const mockRelationships = [
        { sourceRepo: 'repo1', targetRepo: 'repo2', type: 'dependency' }
      ]
  ...
  describe('remove command', () => {
    ...
    it('should handle repository not found', async () => {
      ...
      await expect(handleMultiRepoCommand(['remove', 'nonexistent'])).rejects.toThrow('process.exit')
  ...
  describe('list command', () => {
    it('should list all repositories', async () => {
      ...
      const mockRelationships = [
        { sourceRepo: 'repo1', targetRepo: 'repo2', type: 'dependency', description: 'Uses shared library' }
      ]
  ...
  describe('relate command', () => {
    it('should add relationship successfully', async () => {
      ...
      expect(mockManager.addRelationship).toHaveBeenCalledWith('repo1', 'repo2', 'dependency', 'API client library')
    ...
    it('should default to related type if invalid type provided', async () => {
      ...
      expect(mockManager.addRelationship).toHaveBeenCalledWith('repo1', 'repo2', 'related', undefined)
    })
    ...
    it('should handle relationship errors', async () => {
      ...
      await expect(handleMultiRepoCommand(['relate', 'repo1', 'repo2'])).rejects.toThrow('process.exit')
  ...
  describe('context command', () => {
    it('should display cross-repository context', async () => {
      const mockContext = {
        repositories: [
          ...
          { id: 'repo2', name: 'shared-lib', path: '/test/shared-lib', metadata: { description: 'Shared utilities' } }
        ],
        relationships: [
          { sourceRepo: 'repo1', targetRepo: 'repo2', type: 'dependency', description: 'Uses shared utils' }
        ],
        sharedConfigs: [
          { name: '.eslintrc', path: '.eslintrc', repos: ['repo1', 'repo2'] }
        ]
      }
    ...
    it('should handle context errors', async () => {
      ...
      await expect(handleMultiRepoCommand(['context', '/nonexistent'])).rejects.toThrow('process.exit')
  ...
  describe('export command', () => {
    ...
    it('should handle export errors', async () => {
      ...
      await expect(handleMultiRepoCommand(['export', '/readonly/config.json'])).rejects.toThrow('process.exit')
  ...
  describe('import command', () => {
    ...
    it('should handle import errors', async () => {
      ...
      await expect(handleMultiRepoCommand(['import', 'missing.json'])).rejects.toThrow('process.exit')
