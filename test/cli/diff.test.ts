...
describe('handleDiffCommand', () => {
  ...
  describe('apply command', () => {
    it('should apply changes when confirmed', async () => {
      ...
      expect(mockReadline.question).toHaveBeenCalledWith('Apply these changes? (y/N): ', expect.any(Function))
      expect(mockDiffPreviewer.applyChanges).toHaveBeenCalled()
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Successfully applied changes'))
  ...
  describe('discard command', () => {
    it('should discard changes when confirmed', async () => {
      ...
      expect(mockReadline.question).toHaveBeenCalledWith('Discard 2 pending changes? (y/N): ', expect.any(Function))
