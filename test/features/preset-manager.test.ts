...
describe('PresetManager', () => {
  ...
  describe('createPreset', () => {
    ...
    it('should save metadata after creation', () => {
      ...
      const metadataWrite = writeCalls.find(call => 
  ...
  describe('applyPreset', () => {
    it('should apply preset configuration', async () => {
      ...
      const writeCall = vi.mocked(fs.writeFileSync).mock.calls.find(
        call => call[0].toString().includes('config.yaml')
      )
    ...
    it('should throw error for non-existent preset', async () => {
      await expect(manager.applyPreset('non-existent'))
  ...
  describe('exportPreset', () => {
    ...
    it('should throw error for non-existent preset', () => {
      expect(() => manager.exportPreset('non-existent', '/test/export.yaml'))
        .toThrow('Preset non-existent not found')
    })
  })
  ...
  describe('importPreset', () => {
    ...
    it('should validate imported preset', () => {
      vi.mocked(fs.existsSync).mockReturnValue(true)
      vi.mocked(fs.readFileSync).mockReturnValue(yaml.stringify({
        // Missing required fields
        description: 'Invalid'
      }))
      
      expect(() => manager.importPreset('/test/invalid.yaml'))
    ...
    it('should throw error if file not found', () => {
      ...
      expect(() => manager.importPreset('/test/missing.yaml'))
  ...
  describe('toggleFavorite', () => {
    ...
    it('should throw error for non-existent preset', () => {
      expect(() => manager.toggleFavorite('non-existent'))
        .toThrow('Preset non-existent not found')
    })
  })
  ...
  describe('duplicatePreset', () => {
    ...
    it('should throw error for non-existent preset', () => {
      expect(() => manager.duplicatePreset('non-existent', 'Copy'))
        .toThrow('Preset non-existent not found')
    })
  })
