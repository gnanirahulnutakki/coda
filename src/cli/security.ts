...
export async function handleSecurityCommand(args: string[]): Promise<void> {
  ...
  if (!command) {
    ...
    console.log(
      '  coda security scan [directory] [--exclude=pattern] [--include=pattern] [--format=json|csv|html]  - Scan directory for security issues',
    )
    console.log(
      '  coda security list [--limit=N]                                                                  - List recent scan results',
    )
    console.log(
      '  coda security show <scan-id>                                                                    - Show detailed scan results',
    )
    console.log(
      '  coda security export <scan-id> <output-file>                                                    - Export scan results',
    )
    console.log(
      '  coda security delete <scan-id>                                                                  - Delete scan results',
    )
    console.log(
      '  coda security rules                                                                             - List security rules',
    )
    return
  }
...
async function listScans(scanner: SecurityScanner, args: string[]): Promise<void> {
  try {
    ...
    history.forEach((scan, index) => {
      ...
      console.log(
        `   📊 Issues: \\x1b[31m${scan.criticalIssues}\\x1b[0m critical, \\x1b[33m${scan.highIssues}\\x1b[0m high, ${scan.totalIssues} total`,
      )
      console.log()
    })
  } catch (error) {
...
async function showRules(): Promise<void> {
  ...
  const rulesBySeverity = SECURITY_RULES.reduce(
    (acc, rule) => {
  ...
  console.log(
    `\\x1b[90mFor more information about CWE codes, visit: https://cwe.mitre.org/\\x1b[0m`,
  )
}
