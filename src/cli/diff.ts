...
async function addFileChange(previewer: DiffPreviewer, args: string[]): Promise<void> {
  ...
  try {
    ...
    log(
      `📊 Pending changes: ${stats.totalFiles} files (+${stats.totalAdditions} -${stats.totalDeletions})`,
    )
  } catch (error) {
...
async function applyChanges(previewer: DiffPreviewer): Promise<void> {
  ...
  try {
    ...
    console.log(
      `  \x1b[32m+${stats.totalAdditions}\x1b[0m \x1b[31m-${stats.totalDeletions}\x1b[0m\n`,
    )
