...
export async function handleCostsCommand(args: string[]): Promise<void> {
  ...
  if (!command) {
    ...
    console.log('  coda costs stats [--provider=X] [--project=X] [--days=N]  - Show usage statistics')
    console.log('  coda costs limits [--daily=X] [--weekly=X] [--monthly=X]   - Set or view cost limits')
    console.log('  coda costs history [--limit=N]                            - Show recent usage history')
    console.log('  coda costs estimate <provider> <input> <output> [model]   - Estimate cost for token usage')
    console.log('  coda costs export <file> [--start=YYYY-MM-DD] [--end=YYYY-MM-DD] - Export usage data')
    console.log('  coda costs top [--limit=N]                                - Show most expensive commands')
    console.log('  coda costs pricing                                        - Show current pricing information')
    return
  }
...
async function showStats(costTracker: CostTracker, args: string[]): Promise<void> {
  try {
    ...
    console.log(`  📊 Average Tokens/Request: ${Math.round(stats.averageTokensPerRequest).toLocaleString()}`)
    ...
    if (limits) {
      ...
      if (status.daily) {
        const color = status.daily.exceeded ? '\\x1b[31m' : status.daily.percentage > 80 ? '\\x1b[33m' : '\\x1b[32m'
        console.log(`  📅 Daily: ${color}${formatCurrency(status.daily.used)}\\x1b[0m / ${formatCurrency(status.daily.limit)} (${Math.round(status.daily.percentage)}%)`)
      }
      
      if (status.weekly) {
        const color = status.weekly.exceeded ? '\\x1b[31m' : status.weekly.percentage > 80 ? '\\x1b[33m' : '\\x1b[32m'
        console.log(`  📅 Weekly: ${color}${formatCurrency(status.weekly.used)}\\x1b[0m / ${formatCurrency(status.weekly.limit)} (${Math.round(status.weekly.percentage)}%)`)
      }
      
      if (status.monthly) {
        const color = status.monthly.exceeded ? '\\x1b[31m' : status.monthly.percentage > 80 ? '\\x1b[33m' : '\\x1b[32m'
        console.log(`  📅 Monthly: ${color}${formatCurrency(status.monthly.used)}\\x1b[0m / ${formatCurrency(status.monthly.limit)} (${Math.round(status.monthly.percentage)}%)`)
      }
    }
    ...
    if (!dailyArg && !weeklyArg && !monthlyArg) {
      ...
      if (Object.keys(status).length > 0) {
        ...
        if (status.daily) {
          ...
          console.log(`📅 Today: ${color}${formatCurrency(status.daily.used)}\\x1b[0m (${Math.round(status.daily.percentage)}% of limit)`)
        }
        if (status.weekly) {
          const color = status.weekly.exceeded ? '\\x1b[31m' : '\\x1b[32m'
          console.log(`📅 This Week: ${color}${formatCurrency(status.weekly.used)}\\x1b[0m (${Math.round(status.weekly.percentage)}% of limit)`)
        }
        if (status.monthly) {
          const color = status.monthly.exceeded ? '\\x1b[31m' : '\\x1b[32m'
          console.log(`📅 This Month: ${color}${formatCurrency(status.monthly.used)}\\x1b[0m (${Math.round(status.monthly.percentage)}% of limit)`)
        }
      }
...
async function showHistory(costTracker: CostTracker, args: string[]): Promise<void> {
  try {
    ...
    sessions.reverse().forEach((session, index) => {
      ...
      console.log(`${index + 1}. ${statusIcon} [${time}] ${providerColor}${session.provider}\\x1b[0m`)
...
async function showTopCommands(costTracker: CostTracker, args: string[]): Promise<void> {
  try {
    ...
    topCommands.forEach((cmd, index) => {
      ...
      console.log(`${index + 1}. \\x1b[33m${cmd.command}\\x1b[0m (${providerColor}${cmd.provider}\\x1b[0m)`)
...
async function showPricing(): Promise<void> {
  ...
  console.log(`\\x1b[90m* Prices are subject to change. Check provider documentation for latest rates.\\x1b[0m`)
}
