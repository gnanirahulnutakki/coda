...
export async function handleDoctorCommand(args: string[]): Promise<void> {
  ...
  console.log(`  PATH: ${process.env.PATH?.split(pathSeparator).slice(0, 3).join(pathSeparator)}...`)
  ...
  try {
    ...
    const highPriorityMissing = Object.values(AI_PROVIDERS)
      .filter(p => p.priority === 'high' && !installedIds.has(p.id))
