...
async function searchDocumentation(indexer: DocumentationIndexer, args: string[]): Promise<void> {
  ...
  try {
    ...
    results.forEach((result, index) => {
      ...
      console.log(
        `${index + 1}. \\x1b[33m${chunk.metadata.file}\\x1b[0m (Lines ${chunk.metadata.lineStart}-${chunk.metadata.lineEnd}) - \\x1b[32m${score}% match\\x1b[0m`,
      )
...
async function findSimilar(
  ...
  try {
    ...
    console.log(
      `\\x1b[33mFound ${similarChunks.length} similar chunks for ${relativePath}:${lineNumber}:\\x1b[0m\\n`,
    )
    ...
    similarChunks.forEach((result, index) => {
      ...
      console.log(
        `${index + 1}. \\x1b[33m${chunk.metadata.file}\\x1b[0m (Lines ${chunk.metadata.lineStart}-${chunk.metadata.lineEnd}) - \\x1b[32m${score}% similar\\x1b[0m`,
      )
