...
export class WorkflowTemplateManager {
        ...
        steps: [
          {
            ...
            prompt:
              'Analyze the code in {{targetFile}} and identify all functions, classes, and methods that need unit tests. List them with their expected behaviors.',
          ...
          },
          {
            ...
            prompt:
              'Create a test file for {{targetFile}} with proper imports and describe blocks for each component identified in the previous step.',
          ...
          },
          {
            ...
            prompt:
              'Write comprehensive unit tests for all functions in {{targetFile}}. Include edge cases, error scenarios, and mock external dependencies.',
            successCriteria: [
          ...
          },
          {
            ...
            prompt:
              'Run the test suite and fix any failing tests. Ensure all tests pass with good coverage.',
        ...
        steps: [
          {
            ...
            prompt:
              'Analyze {{targetFile}} and identify complex code blocks that would benefit from being extracted into separate functions. Look for: repeated code, complex conditionals, long methods, and code with unclear intent.',
            variables: { targetFile: '' },
          },
          {
            ...
            prompt:
              'For each identified code block, suggest: 1) A descriptive function name, 2) Parameters needed, 3) Return type, 4) Where to place the function. Ensure the refactoring maintains functionality.',
            requiresConfirmation: true,
          },
          {
            ...
            prompt:
              'Extract the identified code blocks into well-named functions. Update all call sites. Ensure proper parameter passing and return values.',
            variables: { targetFile: '' },
          },
          {
            ...
            prompt:
              'Add JSDoc/TSDoc comments to all newly created functions explaining their purpose, parameters, and return values.',
          },
          {
            name: 'Verify Functionality',
            description: "Ensure the refactoring didn't break anything",
            prompt:
              'Review the refactored code and verify that functionality is preserved. Run any existing tests to ensure nothing is broken.',
            requiresConfirmation: true,
          },
        ],
        variables: {
        ...
        steps: [
          {
            ...
            prompt:
              'Create a {{method}} endpoint at {{path}} that {{description}}. Define the request/response schema, validation rules, and error cases.',
          ...
          },
          {
            ...
            prompt:
              'Implement the endpoint handler with proper request validation, business logic, and response formatting. Include appropriate error handling.',
          ...
          },
          {
            ...
            prompt:
              'Add comprehensive input validation for all request parameters, body fields, and headers. Return appropriate error messages for validation failures.',
          },
          {
            name: 'Add Tests',
            description: 'Create endpoint tests',
            prompt:
              'Write integration tests for the endpoint covering: success cases, validation errors, edge cases, and error scenarios.',
            expectedFiles: ['{{testFile}}'],
          },
          {
            ...
            prompt:
              'Update the API documentation with the new endpoint details including parameters, responses, and example usage.',
          },
        ],
        variables: {
          method: {
        ...
        steps: [
          {
            ...
            prompt:
              'Run a comprehensive security scan on {{targetPath}} using the built-in security scanner. Identify all vulnerabilities grouped by severity.',
          ...
          },
          {
            ...
            prompt:
              'Review the security scan results and prioritize issues by: 1) Severity (critical, high, medium, low), 2) Exploitability, 3) Impact on functionality. Create a remediation plan.',
          },
          {
            name: 'Fix Critical Issues',
            description: 'Fix critical vulnerabilities',
            prompt:
              "Fix all critical severity vulnerabilities identified. Use secure coding practices and industry-standard solutions. Ensure fixes don't break functionality.",
            requiresConfirmation: true,
          },
          {
            ...
            prompt:
              'Fix all high severity vulnerabilities. Apply proper input validation, sanitization, and secure defaults.',
            requiresConfirmation: true,
          },
          {
            name: 'Verify Fixes',
            ...
            prompt:
              'Re-run the security scan to verify all critical and high severity issues are resolved. Run tests to ensure functionality is preserved.',
          },
          {
            name: 'Document Changes',
            description: 'Document security improvements',
            prompt:
              'Document all security fixes made, including: what was vulnerable, how it was fixed, and any configuration changes needed.',
          },
        ],
        variables: {
          targetPath: {
        ...
        steps: [
          {
            ...
            prompt:
              'Based on the issue description: "{{issueDescription}}", create minimal steps to reproduce the problem. Verify you can consistently reproduce it.',
          ...
          },
          {
            ...
            prompt:
              'Gather all relevant information: error messages, logs, stack traces, environment details, and recent changes. Look for patterns or clues.',
          },
          {
            name: 'Form Hypotheses',
            description: 'Develop theories about the cause',
            prompt:
              'Based on the information gathered, form 3-5 hypotheses about what might be causing the issue. Rank them by likelihood.',
          },
          {
            name: 'Test Hypotheses',
            description: 'Test each hypothesis',
            prompt:
              'Systematically test each hypothesis starting with the most likely. Add debug logging, use debugging tools, and isolate components.',
            requiresConfirmation: true,
          },
          {
            name: 'Implement Fix',
            ...
            prompt:
              'Once the root cause is identified, implement a proper fix. Ensure the fix addresses the root cause, not just symptoms.',
            requiresConfirmation: true,
          },
          {
            name: 'Verify Fix',
            ...
            prompt:
              'Verify the fix resolves the issue completely. Test edge cases and ensure no regressions were introduced.',
            requiresConfirmation: true,
          },
          {
            ...
            prompt:
              "Add a test case that would have caught this issue. This ensures the problem doesn't resurface in the future.",
          },
        ],
        variables: {
          issueDescription: {
  ...
  private validateTemplate(template: any): template is WorkflowTemplate {
    return (
      ...
      template.steps.every(
        (step: any) => typeof step.name === 'string' && typeof step.prompt === 'string',
      )
    )
  }
  ...
  updateTemplate(
    id: string,
    updates: Partial<Omit<WorkflowTemplate, 'id' | 'created'>>,
  ): WorkflowTemplate {
  ...
  getRecentExecutions(limit: number = 10): WorkflowExecution[] {
    try {
      const files = fs
        .readdirSync(this.executionsDir)
