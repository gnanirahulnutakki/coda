...
export class DocumentationIndexer {
  ...
  private chunkContent(content: string, filePath: string, fileHash: string, lastModified: number): DocumentChunk[] {
  ...
  async indexDocumentation(options: { force?: boolean; files?: string[] } = {}): Promise<{ indexed: number; updated: number; errors: number }> {
    if (!this.index) {
      throw new Error('No project initialized')
    }
  ...
  private getContext(chunk: DocumentChunk): { before?: string; after?: string } {
    ...
    try {
      ...
      const before = lines
        .slice(Math.max(0, chunk.metadata.lineStart - contextLines - 1), chunk.metadata.lineStart - 1)
        .join('\n')
